...
the following structure
    ...
    {"id": "", "revid": "", "timestamp": "", "url": "", "title": "", "text": "..."}
...
min_days_stable_page_version = 10
...
def load_templates(file: TextIO, output_file: Optional[str] = None) -> int:
    """
        ...
        if tag == 'page':
            ...
            if not output_file and not templateNamespace:  # do not know it yet
                ...
                if colon > 1:
                    ...
                    HistoryExtractor.templatePrefix = title[:colon + 1]
            ...
            if title.startswith(HistoryExtractor.templatePrefix):
                define_template(title, page)
                templates += 1
            # save templates and modules to file
            if output_file and (title.startswith(HistoryExtractor.templatePrefix) or
...
def collect_pages(text: TextIO) -> Iterator[tuple[str, str, str, str, list[Any]]]:
    ...
    for line in text:
        ...
        if tag == 'page':
            ...
            colon = title.find(':')
            if (colon < 0 or title[:colon] in acceptedNamespaces) and last_revid != revid and \
    ...
    for line in input:
        ...
        if tag == 'base':
            ...
            if re.search('key="10"', line):
                ...
                HistoryExtractor.templatePrefix = templateNamespace + ':'
...
def extract_process(jobs_queue: Queue, output_queue: Queue, html_safe: bool) -> None:
    ...
    """
    while True:
        job = jobs_queue.get()  # job is (id, revid, timestamp, urlbase, title, page)
        if job:
            out = StringIO()  # memory buffer
            HistoryExtractor(*job[:-1]).extract(out, html_safe)  # (id, revid, timestamp, urlbase, title, page)
            text = out.getvalue()
...
def main() -> None:
    ...
    HistoryExtractor.keepLinks = args.links
    HistoryExtractor.HtmlFormatting = args.html
    if args.html:
        HistoryExtractor.keepLinks = True
    HistoryExtractor.to_json = args.json
    ...
    if not HistoryExtractor.keepLinks:
        ignoreTag('a')
    ...
    if args.article:
        ...
        with open(input_file) as input:
            for id, revid, timestamp, title, page in collect_pages(input):
                HistoryExtractor(id, revid, timestamp, urlbase, title, page).extract(sys.stdout)
        return
